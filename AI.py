...
class PersonaBindingDataset(Dataset):
    def __init__(self, words, w2i, methods, scenarios, seq_len):
        self.samples = []
        for i in range(len(words) - seq_len):
            ...
            self.samples.append((torch.tensor([w2i[w] for w in ctx]), torch.tensor(w2i[target]), torch.tensor(weight)))
...
if __name__ == "__main__":
    try: local_raw = open(input("Filename: "), "r", encoding="utf-8").read().lower().split()
    except: local_raw = requests.get("https://raw.githubusercontent.com/karpathy/char-rnn/master/data/tinyshakespeare/input.txt").text.lower().split()
    ...
    loader = DataLoader(PersonaBindingDataset(all_words, w2i, methods, scenarios, SEQ_LEN), batch_size=BATCH_SIZE, shuffle=True)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for x, y, w in pbar:
            ...
            loss.backward(); optimizer.step(); pbar.set_postfix(loss=f"{loss.item():.3f}")
