...
# --- Load corpus ---
print("="*60)
print("DATASET SENTENCE REWRITER")
print("="*60)
...
print(f"Extracted {len(sentences)} sentences from dataset")

# Build model
ngram_model, context_map = build_context_aware_ngram_model(tokens, n=N_GRAM_ORDER)
model_keys = list(ngram_model.keys())
print(f"N-gram model built with {len(model_keys):,} keys.")
...
output_filename = "rewritten_dataset.txt"
print(f"\nRewriting {len(sentences)} sentences to '{output_filename}'...")

with open(output_filename, 'w', encoding='utf-8') as outfile:
    for idx, original_sentence in enumerate(sentences, 1):
        # Extract seed from original sentence
        seed_tokens = re.findall(r'\b\w+\b', original_sentence.lower())
        
        if len(seed_tokens) < N_GRAM_ORDER:
            continue
            
        # Use first N words as seed
        start_key = tuple(seed_tokens[:N_GRAM_ORDER])
        
        # Find valid starting key
        if start_key not in ngram_model:
            similar = [k for k in model_keys if any(w in k for w in start_key)]
            start_key = similar[0] if similar else model_keys[np.random.randint(len(model_keys))]
        
        # Generate rewritten version
        stream = context_aware_inference_stream(
            ngram_model, context_map, model_keys, X, clf, start_key, tokens, 
            max_length=len(seed_tokens) + 20  # Slightly longer than original
        )
        
        rewritten = list(start_key)
        for _ in range(len(seed_tokens)):
            try:
                rewritten.append(next(stream))
            except StopIteration:
                break
        
        # Write to file
        rewritten_text = ' '.join(rewritten).capitalize() + '.'
        outfile.write(f"{rewritten_text}\n")
        
        # Progress indicator
        if idx % 50 == 0:
            print(f"Progress: {idx}/{len(sentences)} sentences rewritten")
            outfile.flush()

print(f"\n✓ Complete! Rewritten dataset saved to '{output_filename}'")
